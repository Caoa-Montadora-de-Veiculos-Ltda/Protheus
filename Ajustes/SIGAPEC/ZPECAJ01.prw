...
//		oBrowse:bLDblClick 	:= {|| ZPECAJ01AV(_ObrW:nAt,01][14],.F.) }                               
	_ObrW:AddButton("Visualiza Orçamento"  	, { || ZPECAJ01AV(@_ObrW)  },,,, .F., 2 )  //função no ZPECFUNA
   //Ativamos a classe
    _ObrW:Refresh(.T.)
	_ObrW:Activate()
End SEQUENCE
...
Begin Sequence
	...
	While (_cAlias)->(!Eof())
		...
		FwMsgRun(,{ |_oSay| ZPECAJ01TR(_nReg, (_cAlias)->UPD_B2REC, (_cAlias)->ZD1_COD, (_cAlias)->UPD_QTCONF, (_cAlias)->UPD_SLDIT,_nRegProcessado, @_oSay) }, "Ajustustando armazém de Recebimento ZD1", "Aguarde...")  
