...
Begin Sequence
	...
	aAdd(_aSays,OemToAnsi("da empresa CAOA PEÇAS HD.")) 
	...
	aAdd(_aButtons, { 5,.T.,{|o| ParamBox(_aPar,_cTitle,@_aRet,,,.T.,,,,"ZGENFTE90",.T.,.T.) 		}})
	...
	FwMsgRun(,{ |_oSay| ZGENF90PR(_aRet, @_oSay ) }, "Gerando Notas Fiscais de Entada", "Aguarde...")  //Integração / Aguarde
	
End Sequence
Return Nil


Static Function ZGENF90PR(_aRet, _oSay)
...
Begin Sequence
	...
	if !SA2->( MsSeek(FwXFilial("SA2") + _cCodFor + _cLojaFor ))
		Help( " ", 1,"ZGENF90PR", ,'Fornecedor '+_cCodFor+'-'+_cLojaFor+' não cadastrado não será gerado o Doc.Entrada. ',1)  
		_lRet := .F.
		Break
	Endif
	...
	EndSql
		...
		// SF2.F2_FILIAL      = %Exp:cFilGNF%      //'901001HAD1'
