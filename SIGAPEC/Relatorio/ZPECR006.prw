...
Static Function ReportPrint(oReport)
    ...
    cQuery += CRLF + "			  WHERE SX5.X5_FILIAL  = '" + FWxFilial('SX5') + "' "
    cQuery += CRLF + "				AND SX5.X5_TABELA  = '0A' "
    ...
    cQuery += CRLF + "		LEFT JOIN 	( "
    cQuery += CRLF + "					SELECT PROD, LISTAGG(SLD_WIS,'/') WITHIN GROUP (ORDER BY PROD) QTDEWIS FROM"
    cQuery += CRLF + "					(SELECT E.CD_PRODUTO PROD, E.ARMAZEM ||'='|| NVL(SUM(E.QT_ESTOQUE - E.QT_RESERVA_SAIDA - E.QT_TRANSITO_SAIDA),0) SLD_WIS"
    cQuery += CRLF + "	                FROM " + _cConectWis + " E "
    cQuery += CRLF + "					GROUP BY E.CD_PRODUTO, E.ARMAZEM HAVING SUM(E.QT_ESTOQUE - E.QT_RESERVA_SAIDA - E.QT_TRANSITO_SAIDA) > 0 "
    cQuery += CRLF + "					)GROUP BY PROD "
