...
Begin Sequence
	...
	*/
    ...
    _cQuery += "     			WHEN TRUNC(MONTHS_BETWEEN(TO_DATE( '" + DtOs(_dDataCurva) + "' , 'YYYYMMDD' ) , TO_DATE(D2_EMISSAO , 'YYYYMMDD' ))) + 1 =  08 AND SUM(NVL(SD2.D2_QUANT,0)) > 0 THEN 11 " + CRLF   	
	...
	*/
    ...
    _cQuery += " 		LEFT JOIN	(	SELECT 	 SB2.B2_COD  "+ CRLF
    _cQuery += "  							,  SUM(NVL(SB2.B2_QATU,0)) AS SALDO_ESTOQUE "+ CRLF 
    _cQuery += "							, CASE " + CRLF
	_cQuery += "								WHEN SUM(NVL(SB2.B2_QATU,0)) <> 0 " + CRLF
	_cQuery += "								THEN SUM(NVL(SB2.B2_VATU1,0)) / SUM(NVL(SB2.B2_QATU,0)) " + CRLF
	_cQuery += "								ELSE 0 " + CRLF
	_cQuery += "						  END AS  CUSTO_UNITARIO " + CRLF
...
//    _cQuery += "     AND SZO.D_E_L_E_T_ = ' ' " + CRLF
    ...
    _cQuery += "						, CASE " + CRLF
	_cQuery += "								WHEN SUM(NVL(B2_QATU,0)) <> 0 " + CRLF
	_cQuery += "								THEN SUM(NVL(B2_VATU1,0)) / SUM(NVL(B2_QATU,0)) " + CRLF
	_cQuery += "								ELSE 0 " + CRLF
	_cQuery += "						  END AS  CUSTO_UNITARIO " + CRLF
    _cQuery += "  				FROM " + RetSqlName("SB2") + " SB2 " + CRLF 
    _cQuery += "  				WHERE 	SB2.D_E_L_E_T_ 	= ' ' "  + CRLF
    ...
    _cQuery += "  				WHERE   SD2.D_E_L_E_T_ = ' ' "   + CRLF
    _cQuery += "  					AND SD2.D2_FILIAL BETWEEN  '" + _cFilDe + "' AND '" + _cFilAte + "'  "+ CRLF 
    _cQuery += "  					AND SD2.D2_EMISSAO BETWEEN '" + DtOs(_dDataCurva - 365) + "' AND '" + DtOs(_dDataCurva) +  "' " 
    _cQuery += "  					AND SD2.D2_CF IN " + FormatIn(_cCFVenda,";") + " " + CRLF"
...
Static Function ViewDef()
    ...
    Local _oStruSZO     := FWFormStruct(2, "SZO")           // Cria a estrutura a ser usada na View
    Local oView         := Nil

    oView := FWFormView():New()                             //-- Cria View e seta o modelo de dados
    oView:SetModel(oModel)
	oView:AddField('VIEW_SZO',_oStruSZO,'SZOMASTER')        //Adiciona no nosso View um controle do tipo FormFields(antiga enchoice)
    oView:CreateHorizontalBox('BOX_SZO',100)                //-- Seta o dimensionamento de tamanho
    oView:SetOwnerView('VIEW_SZO','BOX_SZO')                //--Amarra a view com as box
    oView:EnableTitleView('VIEW_SZO', "Curva ABC CAOA") 	//-- Define os títulos do cabeçalho
