...
//	_cQuery += CRLF + " SW8.W8_QTDE AS Qtd, "
	...
	_cQuery += CRLF + " FROM  " + RetSqlName("SW8") + " SW8 "
	_cQuery += CRLF + " 	LEFT JOIN " + RetSqlName("SB1") + " SB1 "
	_cQuery += CRLF + " 	    ON SUBSTR(SB1.B1_FILIAL,1,6)  = SUBSTR(SW8.W8_FILIAL,1,6) "
    ...
    _cQuery += CRLF + "        ON  SZM.ZM_FILIAL  = SW8.W8_FILIAL
	...
	_cQuery += CRLF + " 	    ON SW6.W6_FILIAL = SW8.W8_FILIAL "
	_cQuery += CRLF + " 	    AND SW6.W6_HAWB  = SW8.W8_HAWB "
    ...
    _cQuery += CRLF + "    LEFT JOIN " + RetSqlName("SW9") + " SW9 "
	_cQuery += CRLF + " 	    ON SW9.W9_FILIAL = SW8.W8_FILIAL"
    _cQuery += CRLF + "        AND SW9.W9_INVOICE = SW8.W8_INVOICE "       
    _cQuery += CRLF + "        AND SW9.W9_HAWB    = SW8.W8_HAWB  "       
    _cQuery += CRLF + "        AND SW9.D_E_L_E_T_ = ' ' " 

	_cQuery += CRLF + " WHERE   SW8.D_E_L_E_T_ = ' ' AND SW8.W8_FILIAL = '" + xFilial('SW8') + "'"
    ...
    _cQuery += CRLF + " ) TMP"
    _cQuery += CRLF + " GROUP BY (TMP.STATUS,TMP.FILIAL,TMP.PROCESSO,TMP.PEDIDO,TMP.INVOICE,TMP.DATA_INV,TMP.ITEM,TMP.Descricao,TMP.Desc_DI,TMP.Qtd,TMP.Vl_unit,TMP.Vl_Total,TMP.NOTA,TMP.SERIE,TMP.NAVIO,TMP.Container,TMP.Caixa,"
