...
Static Function ZPECF033PR(_aRet, _cCodProd, _oSay )
    ...
    Begin Sequence
        ...
        For _nPos := 1 To Len(_aCpoCab)
            If _aCpoCab[_nPos,Len(_aCpoCab[_nPos])]  //Valida se a coluna irá para o Browse --Campo que contém .T. no array _aCpoCab
     ...
     // _cQuery += "     		, SC7.C7_DATPRF PENTREGA "+ CRLF                         
        ...
        _cQuery += "             ON SF4D1.F4_FILIAL     = '"+FwXFilial("SF4")+"' " + CRLF               
        _cQuery += "     		AND SF4D1.F4_CODIGO     = SD1.D1_TES "      + CRLF           
        _cQuery += "     		AND SF4D1.F4_ESTOQUE    = 'S' "             + CRLF 
        _cQuery += "            AND SF4D1.D_E_L_E_T_    = '  ' "            + CRLF                  
        _cQuery += "        LEFT JOIN "+RetSqlName("SB1")+" SB1 "           + CRLF    
        _cQuery += "             ON SB1.B1_FILIAL       = '"+FwXFilial("SB1")+"' " + CRLF              
        ...
        _cQuery += "              ON SA2.A2_FILIAL      = '"+FwXFilial("SA2")+"' "  + CRLF               
        ...
        _cQuery += "              ON NNR.NNR_FILIAL     = '"+FwXFilial("NNR")+"' "  + CRLF              
        _cQuery += "             AND NNR.NNR_CODIGO     = SD1.D1_LOCAL "            + CRLF  
        _cQuery += "             AND NNR.D_E_L_E_T_     = '  '  "                   + CRLF              
        _cQuery += "     	WHERE "                                                 + CRLF 
        _cQuery += "             SD1.D1_FILIAL      = '"+FwXFilial("SD1")+"' "      + CRLF             
