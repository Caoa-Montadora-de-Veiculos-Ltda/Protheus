...
Static Function ReportPrint(oReport)
    ...
    While (cAliasTMP)->(!EoF()) .And. !oReport:Cancel()
        ...
        oSection:Cell( "FT_BASEICM"  ):SetValue( iif( Alltrim((cAliasTMP)->F2_ESPECIE) <> "RPS",(cAliasTMP)->FT_BASEICM ,0 ) ) //--Base ICMS
        oSection:Cell( "FT_ALIQICM"  ):SetValue( iif( Alltrim((cAliasTMP)->F2_ESPECIE) <> "RPS",(cAliasTMP)->FT_ALIQICM ,0 ) ) //--Aliq. ICMS
        oSection:Cell( "FT_VALICM"   ):SetValue( iif( Alltrim((cAliasTMP)->F2_ESPECIE) <> "RPS",(cAliasTMP)->FT_VALICM  ,0 ) ) //--Valor ICMS
...
Static Function zTmpRadio3()
	...
	cQuery += CRLF + "        FT_VALCONT, FT_DESCONT, F2_FORMUL , D2_CONTA  , D2_NFORI  , D2_SERIORI, D2_PRUNIT, D2_TOTAL  , "
	cQuery += CRLF + "        D2_DESC   , D2_DESCON , FT_CLASFIS, D2_DESCZFP, D2_DESCZFC, D2_TIPO, "
    ...
    If !Empty( alltrim(MV_PAR20) ) .OR. !Empty( alltrim(MV_PAR21))
	   cQuery += CRLF + " 	AND SC6.C6_CHASSI  BETWEEN '" + MV_PAR20 + "' AND '" + MV_PAR21 + "' "
    Endif
	...
	cQuery += CRLF + " ORDER BY SD2.D2_FILIAL, SD2.D2_DOC, SD2.D2_SERIE, SD2.D2_CLIENTE, SD2.D2_LOJA " 						    + CRLF
...
Static Function zSelNfs5()
    ...
    oMarkBrw:SetMenuDef('')
	oMarkBrw:SetFilterDefault("@"+zFilNf5())
