...
Static Function zSelect2(nRadio)
	...
	If nRadio == 1
		If Pergunte( cPergR1	,.T.	)
			...
			If !Empty(cArquivo)
				Processa({|| zRel001B(cArquivo)}	,"Gerando Relatório de Notas Fiscais de Entrada..."	)
			EndIf
		EndIf
	Else
		If Pergunte( cPergR2	,.T.	)
			cArquivo := cGetFile(cExtens,cTitulo,,cMainPath,.F.)
			If !Empty(cArquivo)
				Processa({|| zRel002B(cArquivo)}	,"Gerando Relatório de Notas Fiscais de Saída..."	)
			EndIf
		EndIf
	EndIf
Return()
...
Static Function zRel001B(cArquivo)
	...
	If (cAliasTRB)->(!Eof())
		...
		While (cAliasTRB)->(!EoF())
			...
			nTot11   +=  iif(valtype((cAliasTRB)->FT_BASEPIS)  = "N", (cAliasTRB)->FT_BASEPIS ,0) 				//--Base Pis Apuração
			aCampo11 := (cAliasTRB)->FT_ALIQPIS                                                   				//--Aliq. Pis Apuração
			nTot12   +=  iif(valtype((cAliasTRB)->FT_VALPIS)   = "N", (cAliasTRB)->FT_VALPIS  ,0) 				//--Valor Pis Apuração
			nTot13   +=  iif(valtype((cAliasTRB)->FT_BASECOF)  = "N", (cAliasTRB)->FT_BASECOF ,0) 				//--Base Cofins Apuração
		    aCampo12 := 	(cAliasTRB)->FT_ALIQCOF                                               				//--Aliq. Cofins Apuração
			nTot14   +=  iif(valtype((cAliasTRB)->FT_VALCOF)   = "N", (cAliasTRB)->FT_VALCOF  ,0) 				//--Valor Cofins Apuração
			...
			aCampo25 := (cAliasTRB)->D1_SERIE                                                     				//--Série
			aCampo26 := (cAliasTRB)->F1_ESPECIE                                                   				//--Espécie
			...
			aCampo29 := IIF(Empty(SToD((cAliasTRB)->D1_EMISSAO)),"",SToD((cAliasTRB)->D1_EMISSAO))				//--Dt. de Emissão
			...
			aCampo47 := AllTrim(Posicione("CC2",1, xFilial("CC2")+ cEstCli +PadR( cCodMun ,TamSx3("CC2_CODMUN")[1]) , "CC2_MUN"))    //--Município
			aCampo48 := AllTrim( (cAliasTRB)->F4_TEXTO )   														//--Descrição CFOP
			aCampo49 := (cAliasTRB)->F1_CODNFE    																//--Cód.Verificação
			aCampo50 := cAmbiente    																			//--Ambiente
			nTot29   +=  iif(valtype((cAliasTRB)->FT_BASEIRR)  = "N", (cAliasTRB)->FT_BASEIRR ,0) 				//--Base Irrf Retenção
			aCampo51 := (cAliasTRB)->FT_ALIQIRR   																//--Aliq. Irrf Retenção
	        nTot30   +=  iif(valtype((cAliasTRB)->FT_VALIRR)   = "N", (cAliasTRB)->FT_VALIRR  ,0) 				//--Irrf Retenção
			...
			nTot35   +=  iif(valtype((cAliasTRB)->D1_ABATISS)  = "N", (cAliasTRB)->D1_ABATISS ,0)			    //--Iss Serviços
			...
			nTot38   +=  iif(valtype((cAliasTRB)->D1_AVLINSS)  = "N", (cAliasTRB)->D1_AVLINSS ,0) 				//--Inss Serviços
			...
			nTot41   +=  iif(valtype((cAliasTRB)->FT_BRETPIS)  = "N", (cAliasTRB)->FT_BRETPIS ,0) 				//--Base Pis Retenção
			aCampo55 := 	(cAliasTRB)->FT_ARETPIS   															//--Aliq. Pis Retenção
	        nTot42   +=  iif(valtype((cAliasTRB)->FT_VRETPIS)  = "N", (cAliasTRB)->FT_VRETPIS ,0) 				//--Valor Pis Retenção
			nTot43   +=  iif(valtype((cAliasTRB)->FT_BRETCOF)  = "N", (cAliasTRB)->FT_BRETCOF ,0) 				//--Base Cofins Retenção    
		    aCampo56 := (cAliasTRB)->FT_ARETCOF    																//--Aliq. Cofins Retenção
			nTot44   +=  iif(valtype((cAliasTRB)->FT_VRETCOF)  = "N", (cAliasTRB)->FT_VRETCOF ,0) 				//--Valor Cofins Retenção
			aCampo57 := (cAliasTRB)->F1_MENNOTA    																//--Msgn Nota Fiscal
			aCampo58 :=	cLogInc    																				//--Log. de Inclusão
			aCampo59 :=	cLogAlt    																				//--Log. de Alteração
			aCampo60 :=	cDtLogAlt    																			//--Dt. Log. de Alteração
			...
			IF ( (cAliasTRB)->D1_DOC <> 	cNF ) .OR.  ( (cAliasTRB)->D1_DOC =	cNF .AND. (cAliasTRB)->D1_TES <> cTES )
				...
				oFWMSExcel:AddRow( cAba1	,cTabela1	,{ 	aCampo01,;    //--Cnpj/Cpf 
															...
															nTot09  ,;    //--Base Subst
															nTot10  ,;    //--Valor Subst
															nTot11  ,;    //--Base Pis Apuração
															aCampo11,;    //--Aliq. Pis Apuração
															nTot12  ,;    //--Valor Pis Apuração
															nTot13  ,;    //--Base Cofins Apuração
															aCampo12,;    //--Aliq. Cofins Apuração
															nTot14  ,;    //--Valor Cofins Apuração
															...
															aCampo46,;    //--Estado
															aCampo47,;    //--Município
															aCampo48,;    //--Descrição CFOP
															aCampo49,;    //--Cód.Verificação
															aCampo50,;    //--Ambiente
															nTot29  ,;    //--Base Irrf Retenção
															aCampo51,;    //--Aliq. Irrf Retenção
															nTot30  ,;    //--Irrf Retenção
															nTot31  ,;    //--Base Inss
															aCampo52,;    //--Aliq. Inss
															...
															nTot37  ,;    //--Valor Iss
															nTot38  ,;    //--Inss Serviços
															nTot39  ,;    //--Base Csll
															aCampo54,;    //--Aliq. Csll
												    		nTot40  ,;    //--Valor Csll
															nTot41  ,;    //--Base Pis Retenção
															aCampo55,;    //--Aliq. Pis Retenção
															nTot42  ,;    //--Valor Pis Retenção
															nTot43  ,;    //--Base Cofins Retenção
															aCampo56,;    //--Aliq. Cofins Retenção
															nTot44  ,;    //--Valor Cofins Retenção
															aCampo57,;    //--Msgn Nota Fiscal
															aCampo58,;    //--Log. de Inclusão
													    	aCampo59,;    //--Log. de Alteração
															aCampo60,;    //--Dt. Log. de Alteração
	...
	If (cAliasTRB)->(!Eof())
		...
		While (cAliasTRB)->(!EoF())
			...
			zRel003B(@nVlIPIRegi, @nVlIPIPres, (cAliasTRB)->F2_ESPECIE, (cAliasTRB)->F2_DOC, (cAliasTRB)->F2_SERIE,;
					...
					nTot10   +=  iif(valtype((cAliasTRB)->FT_ICMSRET)  = "N", (cAliasTRB)->FT_ICMSRET ,0) //--Valor Subst
					nTot11   +=  iif(valtype((cAliasTRB)->FT_BASEPIS)  = "N", (cAliasTRB)->FT_BASEPIS ,0) //--Base Pis Apuração
					aCampo12 := (cAliasTRB)->FT_ALIQPIS    //--Aliq. Pis Apuração
					nTot12   +=  iif(valtype((cAliasTRB)->FT_VALPIS)   = "N", (cAliasTRB)->FT_VALPIS  ,0) //--Valor Pis Apuração
					nTot13   +=  iif(valtype((cAliasTRB)->FT_BASECOF)  = "N", (cAliasTRB)->FT_BASECOF ,0) //--Base Cofins Apuração
					aCampo13 := (cAliasTRB)->FT_ALIQCOF	//--Aliq. Cofins Apuração
					nTot14   +=  iif(valtype((cAliasTRB)->FT_VALCOF)   = "N", (cAliasTRB)->FT_VALCOF  ,0) //--Valor Cofins Apuração
					nTot15   +=  iif(valtype((cAliasTRB)->FT_BASEPS3)  = "N", (cAliasTRB)->FT_BASEPS3 ,0) //--Base Pis ST ZFM
					...
					aCAmpo26 := (cAliasTRB)->D2_SERIE   //--Série
					aCAmpo27 := (cAliasTRB)->F2_ESPECIE    //--Espécie
					aCAmpo28 := AModNot( (cAliasTRB)->F2_ESPECIE )    //--Modelo
					aCAmpo29 := IIF( Empty( SToD( (cAliasTRB)->D2_EMISSAO ) ), "", SToD( (cAliasTRB)->D2_EMISSAO ) )    //--Dt. de Emissão
					...
					aCAmpo42 := AllTrim( Posicione("CC2" ,1 ,xFilial("CC2") + cEstCli + PadR( cCodMun ,TamSx3("CC2_CODMUN")[1] ) , "CC2_MUN" ) )   //--Município
					aCAmpo43 := AllTrim( (cAliasTRB)->F4_TEXTO )   //--Descrição CFOP
					aCAmpo44 := (cAliasTRB)->F2_CODNFE   //--Cód.Verificação
					aCAmpo45 := cAmbiente   //--Ambiente
					nTot29   +=  iif(valtype((cAliasTRB)->FT_BASEIRR)  = "N", (cAliasTRB)->FT_BASEIRR ,0) //--Base Irrf Retenção
					aCAmpo46 := (cAliasTRB)->FT_ALIQIRR   //--Aliq. Irrf Retenção
					nTot30   +=  iif(valtype((cAliasTRB)->FT_VALIRR)   = "N", (cAliasTRB)->FT_VALIRR  ,0) //--Irrf Retenção
					nTot31   +=  iif(valtype((cAliasTRB)->FT_BASEINS)  = "N", (cAliasTRB)->FT_BASEINS ,0) //--Base Inss
					...
					nTot35   +=  iif(valtype((cAliasTRB)->D2_ABATISS)  = "N", (cAliasTRB)->D2_ABATISS ,0) //--Iss Serviços
					...
					nTot40   +=  iif(valtype((cAliasTRB)->FT_VALCSL)   = "N", (cAliasTRB)->FT_VALCSL  ,0) //--Valor Csll
					nTot41   +=  iif(valtype((cAliasTRB)->FT_BRETPIS)  = "N", (cAliasTRB)->FT_BRETPIS ,0) //--Base Pis Retenção
        			aCampo50 :=	IIF( (cAliasTRB)->FT_BRETPIS > 0, (cAliasTRB)->FT_ARETPIS, 0 )    //--Aliq. Pis Retenção
    				nTot42   +=  iif(valtype((cAliasTRB)->FT_VRETPIS)  = "N", (cAliasTRB)->FT_VRETPIS ,0) //--Valor Pis Retenção
					nTot43   +=  iif(valtype((cAliasTRB)->FT_BRETCOF)  = "N", (cAliasTRB)->FT_BRETCOF ,0) //--Base Cofins Retenção    
					aCampo51 := IIF( (cAliasTRB)->FT_BRETCOF > 0, (cAliasTRB)->FT_ARETCOF, 0 )    //--Aliq. Cofins Retenção
 					nTot44   +=  iif(valtype((cAliasTRB)->FT_VRETCOF)  = "N", (cAliasTRB)->FT_VRETCOF ,0) //--Valor Cofins Retenção
					aCampo52 := cLogInc   //--Log. de Inclusão
					aCampo53 := cLogAlt    //--Log. de Alteração
					aCampo54 := cDtLogAlt    //--Dt. Log. de Alteração
					...
					nTot47   +=  iif(valtype((cAliasTRB)->F3_ISENIPI)  = "N", (cAliasTRB)->F3_ISENIPI ,0) //--IPI Isento
					nTot48   +=  iif(valtype((cAliasTRB)->F3_OUTRIPI)  = "N", (cAliasTRB)->F3_OUTRIPI ,0) //--IPI Outros
					aCampo59 := cTransp    //--Cód. Transportadora
			...
			IF ( (cAliasTRB)->D2_DOC <> 	cNF ) .or. ((cAliasTRB)->D2_DOC = cNF .and. (cAliasTRB)->D2_TES <> 	cTES )
				...
				oFWMSExcel:AddRow( cAba1	, cTabela1	, { aCampo01,;    //--Cnpj/Cpf
															...
															nTot10  ,;    //--Valor Subst
															nTot11  ,;    //--Base Pis Apuração
															aCampo12,;    //--Aliq. Pis Apuração
															nTot12  ,;    //--Valor Pis Apuração
															nTot13  ,;    //--Base Cofins Apuração
															aCampo13,;	  //--Aliq. Cofins Apuração
															nTot14  ,;	  //--Valor Cofins Apuração
															nTot15  ,;    //--Base Pis ST ZFM
															...
															aCampo41,;    //--Estado
															aCampo42,;    //--Município
															aCampo43,;    //--Descrição CFOP
															aCampo44,;    //--Cód.Verificação
															aCampo45,;    //--Ambiente
															nTot29  ,;    //--Base Irrf Retenção
															aCampo46,;    //--Aliq. Irrf Retenção
															nTot30  ,;    //--Irrf Retenção
															nTot31  ,;    //--Base Inss
															aCampo47,;    //--Aliq. Inss
															nTot32  ,;    //--Inss Recolhido
															...
															nTot40  ,;    //--Valor Csll
															nTot41  ,;    //--Base Pis Retenção
															aCampo50,;    //--Aliq. Pis Retenção
															nTot42  ,;    //--Valor Pis Retenção
															nTot43  ,;    //--Base Cofins Retenção
															aCampo51,;    //--Aliq. Cofins Retenção
															nTot44  ,;    //--Valor Cofins Retenção
															aCampo52,;    //--Log. de Inclusão
															aCampo53,;    //--Log. de Alteração
															aCampo54,;    //--Dt. Log. de Alteração
															aCampo55,;    //--Tipo Venda
