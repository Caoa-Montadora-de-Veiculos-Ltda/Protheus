...
User Function ZCSP_NFESEFAZ()
	...
	Local aAreaVV1      := {}
	Local aAreaVV2      := {}
	Local cEicMsg1      := SuperGetMv('CMV_EICMS1',.F.,"101/102/201/202/203/204/205")
...
// NFE Importação
	...
	dbSelectArea("SYT")
	aAreaSYT := GetArea()
	SYT->(DbSetOrder(1)) // YT_FILIAL+YP_COD_IMP
...
//Busca Mensagem no Pedido notas de saída
	If aNota[5] $"N/B" .And. Alltrim(aNota[4]) == "1"
		...
		If SD2->(dbSeek(xFilial("SD2")+aNota[2]+aNota[1]+cCodCli+cLoja))
			...
			//CMsgRetirada := " Itens  coletadas no end: Av. Ceci,1649 - MZ MOD. 06, Sitio Mutinga, CEP 06460-120 Barueri-SP, end. onde esta parte do estoque devido operacao de mudanca de endereco."
...
*/	
	...
	If Len(aEntrega) > 0
		cEntrega :=  " Nome: "+Alltrim(aEntrega[09])+ " Local Entrega: "+Alltrim(aEntrega[02])+" "+Alltrim(aEntrega[03])+;
	...
	If SW6->(DbSeek(xFilial("SW6") + cHawb ))
		If SYT->(DbSeek(xFilial("SYT") + SW6->W6_IMPORT))
			cEntrega := " LOCAL DA ENTREGA: " + Alltrim(SYT->YT_NOME)+ "- ENDERECO: "+Alltrim(SYT->YT_ENDE)+" "+ALLTRIM(STR(SYT->YT_NR_END))+;
						", "+Alltrim(SYT->YT_BAIRRO)+","+Alltrim(SYT->YT_CIDADE)+"-"+Alltrim(SYT->YT_ESTADO) +" CEP: " +TRANSFORM(SYT->YT_CEP,"@R 99999-999") +;
						" CNPJ: "+ TRANSFORM(SYT->YT_CGC, "@R 99.999.999/9999-99") + " INSC. ESTADUAL: "+ TRANSFORM(SYT->YT_INSCR_E, "@R 999.999.999.999")
		EndIf
	Endif
	cMensCli := cMensCli + Upper(cEntrega) 
...
//ConOut('aNota[5] $ "N" .And. Alltrim(aNota[4]) == "0" .And. aDest[7] == "99999"')
	If aNota[5] $ "N" .And. Alltrim(aNota[4]) == "0" .And. aDest[7] == "99999" //.And. !(cEicMsg2 $ cMensCli)
		...
		WHERE SF1.%NotDel%
			...
			AND SF1.F1_TIPO = 'N'
			AND SF1.F1_FORMUL = 'S'
			AND (SF1.F1_HAWB <> '      ' OR SF1.F1_XHAWB <> '      ')
		EndSql
		...
		If !Empty( (cAlias)->F1_RECNO )
			...
			If SW6->(DbSeek(xFilial("SW6")+cHawb))
				...
				While !SW6->(eof()) .And. SW6->(W6_FILIAL+W6_HAWB) == xFilial("SW6")+cHawb
					// N. DI.:   ; DT. DI.:
					...
					cMensCli += " PESO BRUTO: " + ALLTRIM(TRANS(SF1->F1_PBRUTO,AVSX3("F1_PBRUTO",6)))
					cMensCli += " PESO LIQUIDO: " + ALLTRIM(TRANS(SF1->F1_PLIQUI,AVSX3("F1_PLIQUI",6)))
					...
					// inicio
					cCmd := CHAR(13) + CHAR(10) + " SELECT DISTINCT D1_XCONT,D1_XCASE FROM " + RetSqlName("SD1")
					...
					(cQry)->(DbEval({|| Aadd(aCont, {Alltrim((cQry)->D1_XCONT),Alltrim((cQry)->D1_XCASE)})} , {|| .T.}))

					(cQry)->(DbCloseArea())

					If Len(aCont) > 0
						cCmd := ""
						aEval(aCont, {|it| cCmd +=   it[1] + ", "})
					...
					iif("201" $ cEicMsg1 .AND. SF1->F1_II      > 0 , cMensCli += " I.I.:"     + Alltrim(Transform(SF1->F1_II    , X3Picture("F1_II"     ))),)
					iif("202" $ cEicMsg1 .AND. SF1->F1_IPI     > 0 , cMensCli += " I.P.I.:"   + Alltrim(Transform(SF1->F1_VALIPI  , X3Picture("F1_VALIPI"    ))),)
					iif("203" $ cEicMsg1 .AND. SF1->F1_VALICM  > 0 , cMensCli += " I.C.M.S.:" + Alltrim(Transform(SF1->F1_VALICM  , X3Picture("F1_VALICM" ))),)
					iif("204" $ cEicMsg1 .AND. SF1->F1_VALIMP6 > 0 , cMensCli += " PIS: "     + Alltrim(Transform(SF1->F1_VALIMP6 , X3Picture("F1_VALIMP6"))),)
					iif("205" $ cEicMsg1 .AND. SF1->F1_VALIMP5 > 0 , cMensCli += " COFINS: "  + Alltrim(Transform(SF1->F1_VALIMP5 , X3Picture("F1_VALIMP5"))),)
					if !Empty(SF1->F1_HAWB)
						cHawb := SF1->F1_HAWB
					Else
						cHawb := SF1->F1_XHAWB
					...
					If SWD->(DbSeek(xFilial("SWD")+cHawb))
						...
						While !SWD->(eof()) .And. SWD->(WD_FILIAL+WD_HAWB) == xFilial("SWD")+cHawb
		...
		if !Vazio(cHawb)
			cCmd    := Posicione("SW6", 1, SF1->F1_FILIAL + cHawb, "W6_DI_NUM")
			if !Empty(cCmd)
				dDtDi   := Posicione("SW6", 1, SF1->F1_FILIAL + cHawb, "W6_DTREG_D")
				cDesemb := Posicione("SW6", 1, SF1->F1_FILIAL + cHawb, "W6_LOCALN")
				//cMensFis += "Declaração de Importação: " + Alltrim(cCmd) + "-" + Dtoc(dDtDi) + '-' + alltrim(cDesemb) + '-'
			EndIf
			...
			cMensCli += ' PIS: '    + Alltrim(Transform(SF1->F1_VALIMP6, X3Picture("F1_VALIMP6")))
			cMensCli += ' COFINS: ' + Alltrim(Transform(SF1->F1_VALIMP5, X3Picture("F1_VALIMP5")))
			...
			cCmd := Posicione("SW6", 1, SF1->F1_FILIAL + cHawb, "W6_XTIPIMP")
	...
	If Vazio(GetAdvFVal("SF1", "F1_XMSGADI", xFilial("SF1") + SF1->F1_DOC + SF1->F1_SERIE + SF1->F1_FORNECE + SF1->F1_LOJA , 1, "")) = .F.
		...
		cCmd += CRLF + " LEFT JOIN " + RetSqlName("SF1") + " SF1 ON SF1.D_E_L_E_T_ = ' ' AND W8_FILIAL = F1_FILIAL "
		cCmd += CRLF +  	"AND W8_FORN = F1_FORNECE AND W8_FORLOJ = F1_LOJA AND F1_HAWB = W8_HAWB "
		cCmd += CRLF + " WHERE "
		cCmd += CRLF + " SZM.D_E_L_E_T_ = ' ' "
		cCmd += CRLF + " AND ZM_CONT = '" + cContainer + "' "
		cCmd += CRLF + " AND W8_HAWB = '" + cHawb + "'"
		...
		cMensCli += "  Desembarque: " + Alltrim((cQry)->W8_HAWB) + " Container: " + Alltrim(cContainer) 
		cMensCli += "  Caixa: " + Alltrim(cCaixas) + " Impostos recolhidos na(s) Nota(s) fiscal(is) Ser/Doc: "
