...
//Verificar se altera o posicionamento da SF1/SD1
    IF !Empty(cChassi) .AND. cTipo == 'I' 
		...
		SELECT DISTINCT SD1.D1_FILIAL AS FILIAL,SD1.D1_DOC AS CD5_DOC,SD1.D1_SERIE AS CD5_SERIE,SD1.D1_FORNECE AS CD5_FORNEC,SD1.D1_LOJA AS CD5_LOJA,;
		    SF1.F1_ESPECIE AS CD5_ESPEC,SW6.W6_DI_NUM AS CD5_NDI,SW6.W6_DTREG_D AS CD5_DTDI,SW6.W6_LOCALN AS CD5_LOCDES,SW6.W6_UFDESEM AS CD5_UFDES,;
		...
		If (cAliasCD5)->(!EOF())
			...
			If cAliasTRB->VIA_TRA = 'M'
				cCD5_VTRANS := '1'
			ElseIf cAliasTRB->VIA_TRA = 'A' 
				cCD5_VTRANS := '4'
			EndIf 
			...
			CD5_DTDI   := cAliasCD5->DT_DI 
			CD5_LOCDES := cAliasCD5->LOCAL_NOME
			CD5_UFDES  := cAliasCD5->CD5_UFDES
			CD5_DTDES  := cAliasCD5->CD5_DTDES
			CD5_CODEXP := cAliasCD5->D1_FORNECE
		...
		SELECT SD1.D1_FILIAL AS CD9_FILIAL, SF1.F1_ESPECIE AS CD9_ESPEC, SD1.D1_FORNECE AS CD9_CLIFOR, SD1.D1_LOJA AS CD9_LOJA, SD1.D1_ITEM AS CD9_ITEM, ;
		    ...
		    VV2.VV2_CAPTRA AS CD9_CMKG, VV2_DISEIX AS CD9_DISTEI, VV1.VV1_FABMOD AS CD9_ANOMOD, VV1.VV1_FABMOD AS CD9_ANOFAB,VVC.VVC_TIPCOR,;
		    ...
		    VV2.VV2_CILMOT AS CD9_CILIND, VVC.VVC_GRUCOR AS CD9_CORDE, VV2_COMVEI AS COMVEI
		FROM %table:VV1% VV1 
		LEFT JOIN %table:SD1% SD1 ON SD1.%NotDel% 
			AND SUBSTR(VV1.VV1_FILIAL,1,6) = SUBSTR(SD1.SD1_FILIAL,1,6) 
		...
		LEFT JOIN %table:VVC% VVC ON VVC.%NotDel% 
			AND SUBSTR(VVC.VVC_FILIAL,1,6) = SUBSTR(SD1.SD1_FILIAL,1,6) 
			AND VVC.VVC_CODMAR = VV2.VV2_CODMAR
			AND VVC.VVC_CORVEI = VV2.VV2_COREXT
		LEFT JOIN %table:VVE% VVE ON VVE.%NotDel% 
			AND SUBSTR(VVE.VVE_FILIAL,1,6) = SUBSTR(SD1.SD1_FILIAL,1,6) 
		...
		If (cAliasCD9)->(!EOF())
			...
			If cAliasTRB->VVC_TIPCOR = 0
				cCD9_TPPINT := '1'
			ElseIf cAliasTRB->VVC_TIPCOR = 1
				cCD9_TPPINT := '2'
			ENDIF

			fSX5CV(cAliasCD9->COMVEI)  //De x Para combustíveis 
