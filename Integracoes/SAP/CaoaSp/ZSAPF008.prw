...
user function ZSAPF008( aParam )
	...
	IF lIsBlind
		...
		If !LockByName("ZSAPF008")
			Alert("JOB já em Execução : ZSAPF008 " + DTOC(dDATABASE) + " - " + TIME() )
			RpcClearEnv()
			Return
		EndIf
	EndIF
	...
	cQry += " WHERE  " + CRLF
	cQry += " SZ7.D_E_L_E_T_ = ' ' " + CRLF
	//cQry += " AND (Z7_XSTATUS = 'P' OR (Z7_XSTATUS = 'E' AND Z7_XDTINC >= '"+dTos(dDataBase-nDiasErro)+"') OR (Z7_XSTATUS = 'A' AND Z7_XDTINC >= '"+dTos(dDataBase-nDiasAguar)+"' AND Z7_XDTINC < '"+dTos(dDataBase-1)+"')) "
	cQry += "	AND (Z7_XSTATUS = 'P' OR (Z7_XSTATUS = 'E' AND Z7_XDTINC >= '"+dTos(dDataBase-nDiasErro)+"')) " + CRLF
	//cQry += " AND Z7_XSTATUS = 'P' "
	...
	while !(cAliasQry)->(Eof())
		...
		*/	
						...
						If SD2->(Recno()) == Val(CV3->CV3_RECORI)
							...
							If SF2->(dbSeek(SD2->D2_FILIAL+SD2->D2_DOC+SD2->D2_SERIE+SD2->D2_CLIENTE+SD2->D2_LOJA))
								If !U_ZF08GENSAP(SF2->F2_ESPECIE,SF2->F2_CHVNFE,SF2->F2_FIMP,SF2->F2_DOC,SF2->F2_SERIE)
									U_ZF12GENSAP((cAliasQry)->Z7_FILIAL,(cAliasQry)->Z7_XTABELA,(cAliasQry)->Z7_XCHAVE,(cAliasQry)->Z7_XSEQUEN,"E","[ZSAPF008] - Envio movimento SAP, Nota Fiscal não autorizada na Sefaz. NF :"+Alltrim(SF2->F2_DOC)+"/"+Alltrim(SF2->F2_SERIE)," ")
									(cAliasQry)->(dbSkip())
									Loop
								Endif
							Endif
							...
							If SD1->D1_FORMUL == "S"
								...
								If SF1->(dbSeek(SD1->D1_FILIAL+SD1->D1_DOC+SD1->D1_SERIE+SD1->D1_FORNECE+SD1->D1_LOJA))
									If !U_ZF08GENSAP(SF1->F1_ESPECIE,SF1->F1_CHVNFE,SF1->F1_FIMP,SF1->F1_DOC,SF1->F1_SERIE)
										U_ZF12GENSAP((cAliasQry)->Z7_FILIAL,(cAliasQry)->Z7_XTABELA,(cAliasQry)->Z7_XCHAVE,(cAliasQry)->Z7_XSEQUEN,"E","[ZSAPF008] - Envio movimento SAP, Nota Fiscal não autorizada na Sefaz. NF :"+Alltrim(SF1->F1_DOC)+"/"+Alltrim(SF1->F1_SERIE)," ")
										(cAliasQry)->(dbSkip())
										Loop
									Endif
								Endif	
		...
		Conout(" [SAP] [ZSAPF008] * * * * * Status da integracao * * * * *"									)
		Conout(" [SAP] [ZSAPF008] Inicio.......................: " + cTimeIni + " - " + dToC( dDataBase ) 	)
		Conout(" [SAP] [ZSAPF008] Fim..........................: " + cTimeFin + " - " + dToC( dDataBase ) 	)
		Conout(" [SAP] [ZSAPF008] Tempo de Processamento.......: " + cTimeProc 								)
		Conout(" [SAP] [ZSAPF008] URL..........................: " + cUrl									)
		Conout(" [SAP] [ZSAPF008] HTTP Method..................: " + "GET" 									)
		Conout(" [SAP] [ZSAPF008] Status Http (200 a 299 ok)...: " + allTrim( str( nStatuHttp ) ) 			)
		Conout(" [SAP] [ZSAPF008] Retorno......................: " + allTrim( xPostRet ) 					)
		Conout(" [SAP] [ZSAPF008] * * * * * * * * * * * * * * * * * * * * "									)
		...
		if nStatuHttp >= 200 .and. nStatuHttp <= 299
			...
			If (cAliasQry)->Z7_XOPESAP <> 2 
				If (cAliasCT2)->(!Eof())
					...
					xRet := oWsdl:SetValue( aSimple[nPos][1], _cEmpresa /*SUBSTR((cAliasCT2)->CT2_FILIAL,1,4)*/ )	//empresa
					If !xRet
					...
					xRet := oWsdl:SetValue( aSimple[nPos][1], _cFilial )	//filial do parametro
					If !xRet
				...
				Else
					...
					U_ZF12GENSAP((cAliasQry)->Z7_FILIAL,(cAliasQry)->Z7_XTABELA,(cAliasQry)->Z7_XCHAVE,(cAliasQry)->Z7_XSEQUEN,"E","[ZSAPF008] - Envio movimento SAP, Movimento contabilizado com inconsistência."," ")
					(cAliasQry)->(dbSkip())
					Loop
				EndIf
...
Static Functio xQryCt2(cxFil,cxChav)
...
user function xZSAPF08(xParam1,xParam2,xParam3)

U_ZSAPF008({{xParam1,xParam2,Val(xParam3)}})
