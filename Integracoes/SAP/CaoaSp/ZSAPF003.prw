...
IF lIsBlind
	...
	If !LockByName("ZSAPF003")
		Conout("JOB já em Execução : ZSAPF003 " + DTOC(dDATABASE) + " - " + TIME() )
		RpcClearEnv()
		Return
	EndIf
EndIF
...
While !(cAliasQry)->(EOF())
	...
	*/	
	...
	conout(" [SAP] [ZSAPF003] * * * * * Status da integracao * * * * *"									)
	conout(" [SAP] [ZSAPF003] Inicio.......................: " + cTimeIni + " - " + dToC( dDataBase ) 	)
	conout(" [SAP] [ZSAPF003] Fim..........................: " + cTimeFin + " - " + dToC( dDataBase ) 	)
	conout(" [SAP] [ZSAPF003] Tempo de Processamento.......: " + cTimeProc 								)
	conout(" [SAP] [ZSAPF003] URL..........................: " + cUrl									)
	conout(" [SAP] [ZSAPF003] HTTP Method..................: " + "GET" 									)
	conout(" [SAP] [ZSAPF003] Status Http (200 a 299 ok)...: " + allTrim( str( nStatuHttp ) ) 			)
	conout(" [SAP] [ZSAPF003] Retorno......................: " + allTrim( xPostRet ) 					)
	conout(" [SAP] [ZSAPF003] * * * * * * * * * * * * * * * * * * * * "									)
		...
		*/
		...
		If (cAliasQry)->Z7_XOPESAP <> 2
			...
			If !lSE2
				...
				Else
					cDocSAP := U_ZF07GENSAP(SD2->D2_NFORI) // FormatDocSAP(SF2->F2_DOC)
					cRefSAP := cDocSAP
					cDocSAPDev := U_ZF07GENSAP(SF2->F2_DOC)
				Endif
			Else
			...
			//cDocSAP := U_ZF07GENSAP(lSE2,lDev)
			//A.Carlos - 31/03/22 Removida a lógica de geração das Tags Empresa, Divisão e Filial
			//CMV_SAP001 - Empresa  - 2010
            //CMV_SAP002 - Filial   - 2005
			//CMV_SAP003 - Divisao  - 0509
			aadd( aAux , { "empresa"					, _cEmpresa/*left( allTrim( IIf(!lDev,IIf(!lSE2,SF1->F1_FILIAL,SE2->E2_FILIAL),SF2->F2_FILIAL) ) , 4 )*/ }  )	// tag empresa
			aadd( aAux , { "divisao"					, _cDivisao } )											// tag divisao
			aadd( aAux , { "filial"						, _cFilial/*right( allTrim( IIf(!lDev,IIf(!lSE2,SF1->F1_FILIAL,SE2->E2_FILIAL),SF2->F2_FILIAL) ) , 4 )*/ } )		// tag filial
			...
			If !lAdt
				...
				//memoWrite( "C:\TEMP\ZSAPF003.TXT",cQ )
			...
			While SE2->(!Eof()) .and. cChaveTit == SE2->E2_FILIAL+SE2->E2_FORNECE+SE2->E2_LOJA+SE2->E2_PREFIXO+SE2->E2_NUM
				...
				aadd( aAux , { "empresa"								, _cEmpresa/*left( allTrim( IIf(!lDev,IIf(!lSE2,SF1->F1_FILIAL,SE2->E2_FILIAL),SF2->F2_FILIAL) ) , 4 )*/ } )
				aadd( aAux , { "divisao"								, _cDivisao } )											//	divisao
				...
				/*
				If !lSE2
					if (!lDev .and. !allTrim( SF1->F1_ESPECIE ) $ cFrete) .or. lDev
						//cDocSAP := U_ZF07GENSAP(IIf(!lDev,SD1->D1_PEDIDO,SD2->D2_PEDIDO))
						//aadd( aAux , { "numeroAtribuicao"		, IIf(!lDev,SD1->D1_PEDIDO,SD2->D2_PEDIDO)+"-"+cParc } )
						aadd( aAux , { "numeroAtribuicao"		, Alltrim(cDocSAP)+"-"+cParc } )
						
					else
						//aadd( aAux , { "numeroAtribuicao"		, Alltrim(SF1->F1_DOC) } )
						aadd( aAux , { "numeroAtribuicao"		, Alltrim(cDocSAP)+"-"+cParc } )
					endif
				Else
				...
				*/
				...
				aadd( aAux , { "filial"									, _cFilial/*right( allTrim( IIf(!lDev,IIf(!lSE2,SF1->F1_FILIAL,SE2->E2_FILIAL),SF2->F2_FILIAL) ) , 4 )*/ } )				//	filial
...
user function xZSAPF03(xParam1,xParam2,xParam3)
Default xParam1 := "02"
Default xParam2 := "20200012001"
Default xParam3 := "0"

U_ZSAPF003({{xParam1,xParam2,Val(xParam3)}})
