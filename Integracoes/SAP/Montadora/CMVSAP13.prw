...
IF lIsBlind
	...
	If !LockByName("CMVSAP13")
		Conout("JOB já em Execução : CMVSAP13 " + DTOC(dDATABASE) + " - " + TIME() )
		RpcClearEnv()
		Return
	Else
		Conout("Conexão realizada com sucesso : CMVSAP13 " + DTOC(dDATABASE) + " - " + TIME() )
	EndIf
EndIF
...
While !(cAliasQry)->(EOF())
	...
	*/	
	...
	// se for exclusao e titulo for do EEC, nao prossegue, pois serah enviado pelo cmvsap03
	If (cAliasQry)->Z7_XOPESAP == 2
		If U_ZF01GENSAP()
			(cAliasQry)->(dbSkip())
			LOOP
	...
	conout(" [SAP] [CMVSAP13] * * * * * Status da integracao * * * * *"									)
	conout(" [SAP] [CMVSAP13] Inicio.......................: " + cTimeIni + " - " + dToC( dDataBase ) 	)
	conout(" [SAP] [CMVSAP13] Fim..........................: " + cTimeFin + " - " + dToC( dDataBase ) 	)
	conout(" [SAP] [CMVSAP13] Tempo de Processamento.......: " + cTimeProc 								)
	conout(" [SAP] [CMVSAP13] URL..........................: " + cUrl									)
	conout(" [SAP] [CMVSAP13] HTTP Method..................: " + "GET" 									)
	conout(" [SAP] [CMVSAP13] Status Http (200 a 299 ok)...: " + allTrim( str( nStatuHttp ) ) 			)
	conout(" [SAP] [CMVSAP13] Retorno......................: " + allTrim( xPostRet ) 					)
	conout(" [SAP] [CMVSAP13] * * * * * * * * * * * * * * * * * * * * "									)
	...
	if nStatuHttp >= 200 .and. nStatuHttp <= 299
		...
		*/
		...
		If (cAliasQry)->Z7_XOPESAP <> 2
			...
			aadd( aAux , { "empresa"					, left( allTrim( SE2->E2_FILIAL ) , 4 ) }  )				// tag empresa
			aadd( aAux , { "divisao"					, cDivisao } )											// tag divisao
			aadd( aAux , { "filial"						, right( allTrim( SE2->E2_FILIAL ) , 4 ) } )				// tag filial
			...
			memoWrite( "C:\TEMP\CMVSAP13.TXT",cQ )
			...
			aadd( aAux , { "empresa"								, left( allTrim( SE2->E2_FILIAL ) , 4 ) } )
			aadd( aAux , { "divisao"								, cDivisao } )											//	divisao
			...
			aadd( aAux , { "filial"									, right( allTrim( SE2->E2_FILIAL ) , 4 ) } )				//	filial
...
IF lIsBlind
	Conout("Conexão finalizada com sucesso : CMVSAP13 " + DTOC(dDATABASE) + " - " + TIME() )
	RpcClearEnv()
Endif	
...
user function xCMVSA13(xParam1,xParam2,xParam3)

U_CMVSAP13({{xParam1,xParam2,Val(xParam3)}})
