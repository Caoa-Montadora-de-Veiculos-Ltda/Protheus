...
User Function ZFINF002()
	...
	aAdd(aParamBox,{1 ,"Cliente :" ,cCodCli,"@!",,"SA1","",70,.T.}) 
	aAdd(aParamBox,{1 ,"Loja    :" ,cLojCli,"@!",,"","",70,.T.}) 

    If ParamBox(aParamBox,"Parametros para Consulta de Clientes",@aRet) 
	    cCodCli := aRet[1]
	    cLojCli := aRet[2]
    	MsgRun("Montando Consulta. Aguarde..." , "CAOA" , {|| CursorWait() , ConsCliWnd() , CursorArrow() } )
	EndIF
...
Static Function ConsCliWnd()
	...
	//aCols := CarDados(lFiltro)

	IF lFiltro = .F.
	    aCols := CarDados(cCodCli,cLojCli)
    ENDIF
	...
	*/
	...
	TButton():New(aPosObj1[VW_SEC_FOOTER,3]-54,aPosObj1[VW_SEC_FOOTER,4]-53,"&Filtrar",oDlg,{|| Processa({||   U_Filtrar()},'CAOA','Filtrando. Aguarde...') },50,15,,oBold,.F.,.T.,.F.,,.F.,,,.F. )
	TButton():New(aPosObj1[VW_SEC_FOOTER,3]-36,aPosObj1[VW_SEC_FOOTER,4]-53,"&Excel"  ,oDlg,{|| Processa({|| U_ZGENEXCEL( aTitles, aCols, "Planilha", "Consulta de Clientes - CAOA")},'CAOA','Gerando Relatório. Aguarde...') },50,15,,oBold,.F.,.T.,.F.,,.F.,,,.F. )
...
/*/{Protheus.doc} CarDados
	...
	@history 	26/08/2020	, denis.galvani, Ajustes na passagem de parâmetros (ordem) aos métodos da classe
	/*/
Static Function CarDados(cCodCli,cLojCli,lFiltro)//,cLojCliAte,Filtrar)
	Local cQuery	 := ""
	Local cRaizCNPJ	 := ""
	Local cTipOper	 := ""
	Local cOperCrd	 := ""
	...
	cQuery += "  E.D_E_L_E_T_ = ' ' "
	If !lCliLoj
		cQuery += "  AND SUBSTR(A.A1_CGC,1,8) = '" + cRaizCNPJ + "' "
    ...
    _cChaveInd := "E1_VENCREA"

	Count To nTotal

	If nTotal > 0

	    // Limpa o Variáveis totalizadoras
		IF lFiltro = .T.
			nTotAbto   := 0
			nTotEnca   := 0
			nTotDocs   := 0
	...
	If lFiltro .AND. nTotal > 0
        oGetDados:SetArray(aCols)
	ELSEIf lFiltro .AND. nTotal = 0
	    lFiltro := .F.
		MsgInfo("Não há dados a serem consultados.","[CONSCLI]")
		oDlg:End()
		Return()
	EndIf 
...
// /*/{Protheus.doc} FCon02
	// Função Auxiliar, para apresentar operações de crédito contidas no pedidos de venda.
	// @type 		function
	// @author 		TOTVS - FSW - DWC Consult
	// @since 		24/02/2019
	// @version 	1.0
	// @param 		cFilPed		, character , Filial do Pedido de Venda - campo `C5_FILIAL` e `C6_FILIAL` relacionados
	// @param 		cPedido		, character	, Código do Pedido de Venda - campos `C5_NUM` e `C6_NUM` relacionados
	// @param 		cNumE1		, character	, Número do Título financeiro a receber relacionado ao Pedido - campo `E1_NUM`
	// @return 	character	, Código do "Tipo de Operação" do item do Pedido - campo `C6_XOPER`
	// @deprecated
	// @history 	22/08/2020	, denis.galvani, Inclusão/padronização de documentação de cabeçalho
	// /*/
// Static Function FCon02(cFilPed,cPedido,cNumE1,cTipoE1)
// 	Local cTpOper	:= ""
// 	Local cAliSC6	:= GetNextAlias()
// 	DEFAULT cNumE1  := Space(GetSx3Cache("E1_NUM", "X3_TAMANHO"))
// 	DEFAULT cTipoE1 := Space(GetSx3Cache("E1_TIPO","X3_TAMANHO"))

// 	If Select(cAliSC6) > 0
// 		(cAliSC6)->(DbCloseArea())
// 	EndIf

// 	BeginSql Alias cAliSC6
// 		SELECT 
// 			C6_FILIAL, 
// 			C6_NUM, 
// 			C6_XOPER, 
...
// 		WHERE
// 				C.%NotDel%
// 			AND C.C6_FILIAL = %Exp:cFilPed%
// 			AND C.C6_NUM = %Exp:cPedido%
// 			AND C.C6_NOTA = %Exp:cNumE1%
// 			AND C.C6_SERIE = %Exp:cTipoE1%
// 	EndSql

// 	cTpOper := (cAliSC6)->C6_XOPER
...
// FILTRO
User Function Filtrar()

    IF lFiltro
        MsgInfo("Para gerar novo filtro saia e entre novamente no Browse.","[CONSCLI]") 
        Return Nil
	ENDIF

    lFiltro       := .T.
	cFiltroRet    := BuildExpr("SE1",,cFiltroRet)   //cAliTit
   
    cFiltro  := StrTran(cFiltroRet, "==", "=")
	cFiltro1 := StrTran(cFiltro, '"', "'")

	IF Empty(cFiltro1)
        MsgInfo("Filtro cancelado!!!","[CONSCLI]") 
		lFiltro := .T.
        Return Nil
	ENDIF
	...
	aCols := CarDados(cCodCli,cLojCli,lFiltro)
