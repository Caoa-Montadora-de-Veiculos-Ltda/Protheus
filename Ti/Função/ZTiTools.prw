...
@version 1.0
*/
User Function zTiTools()

//Paramentros para Atalho: " -m -p=u_zTiTools -c=HOM_23608 -e=ABDHDU_HOM_DESENV_23608
// -m = Multi tela
...
//Cria o MsApp
    ...
    oApp:bMainInit:= {|| MsgRun("zTiTools - Inicializando Ambiente de Desenvolvimento","Aguarde...",;
...
Static Function ApQry2Run(cQuery, cAliasExqr, cAliasTst, oPnlQryI, np, oTime, oQtd)
    ...
    Processa({|| PApQry2Run(cQuery, cAliasExqr, cAliasTst, oPnlQryI, np, oTime, oQtd) } , "Executando query...")
...
Static Function PApQry2Run(cQuery, cAliasExqr, cAliasTst, oPnlQryI, np, oTime, oQtd)
    ...
    Begin Sequence
        If ! cComando == "SELECT"
            ...
            ProcRegua( Len(aComando) )

            For nx:= 1 to len(aComando)

                nAtual++
                IncProc("Executando registro " + cValToChar(nAtual) + " de " + cValToChar(Len(aComando)) + "...")
...
ACTIVATE MSDIALOG oDlgX CENTERED ON INIT EnchoiceBar(oDlgX,{|| u_zValidPSW() },{|| u_zClosePSW()})
